...
export class MainGroupController {
    ...
    @ApiQuery({type: String, name: 'lastId', required: false, description: 'messageId I should start with'})
    @ApiParam({type: String, name: 'roomId', required: true, description: 'room Id i will speack with people'})
    @UseGuards(JWTAuthGuard)
    @UseInterceptors(TimersIntercertor)
    @Get(':roomId/messages')
    async initialSetMessageOrOtherData(
        @Req() req: Request, @Param('roomId') roomId: string, @Query('lastId') lastId: string   // Date
        ...
        return await this.maingroupService.getGroupOtherData({roomId: roomId, lastId: lastId})
    }
   ...
   @ApiBody({type: CreateNewGroupSwagger})
    ...
    @ApiParam({type: String, name: 'roomId', required: true, description: 'current group i am locating'})
    @ApiQuery({type: String, name: 'userId', required: true, description: 'user i will add to current group'})
    @UseGuards(JWTAuthGuard)
    @ApiBody({type: AddNewMemberSwagger})
    @UseInterceptors(TimersIntercertor)
    @Patch(':roomId/members')
    async handleAddNewMember(
        @Req() req: Request, @Param('roomId') roomId: string, @Query('userId') userId: string, //memberId 
    @Body(new ZodValidationPipe(addNewMemberGroupSchema)) dto: AddNewMemberGroup): Promise<NotificationTypeDataForClient> {
        if (!req.user) throw new UnauthorizedException('you are not authorizated')
        const authorId = req.user.userId
        return await this.maingroupService.addNewMember({authorId: authorId, roomName: dto.roomName, memberId: Number(userId)})
    }
    ...
    @ApiParam({type: String, name: 'roomId', required: true, description: 'the current room i am locating'})
    @ApiQuery({type: String, name: 'userId', required: true, description: 'user i will delete from the current group'})
    @UseGuards(JWTAuthGuard)
